...
const App = () => {
  ...
  const partition = async (arr, low, high) => {
    let pivot = arr[high];
    setComparingGroup([high]);
    await sleep(animation_speed)
    let i = low - 1;
    let elements_less_than_pivot = []

    for (let j = low; j < high; j++) {
      // let curr_bar = document.getElementById(j);
      // curr_bar.style.backgroundColor = "crimson";
      setComparingGroup([high, j]);
      await sleep(5*animation_speed);
      if (arr[j] < pivot) {
        i++;
        let temp = arr[j];
        arr[j] = arr[i];
        arr[i] = temp;
        elements_less_than_pivot.push(i);
        setComparingGroup([high]);
        setSwappingGroup([i, j]);
        await sleep(2*animation_speed);
        setArr([...arr]);
        await sleep(2*animation_speed);

        setSwappingGroup([]);
        setLessThanPivotGroup([...elements_less_than_pivot]);
        // setLeftRightSortedGroup([[...elements_less_than_pivot], []]);
        // let small_bar = document.getElementById(i);
        // small_bar.style.backgroundColor = "purple";
      }
      // console.log(elements_less_than_pivot);
      // await sleep(100);
  ...
  //   // if (low === high) {
  //   //   await sleep(1000);
  //   //   let new_sorted_group = [...sorted_group];
  //   //   setSortedGroup([...new_sorted_group, low]);
  //   // }
  //   const stack = [];
  ...
  //     setLeftRightSortedGroup([]);
  //     let new_sorted_group = [...sorted_group];
  //     new_sorted_group.push(pivot_index);
  //     setSortedGroup([...new_sorted_group]);
  //     console.log("SortedGroup",[...new_sorted_group])
  ...
  const quickSort = async () => {
    ...
    let new_arr = [...arr];
    let corrected_indexes = []
    const stack = [];
    stack.push([0, new_arr.length-1]);
    while(stack.length != 0){
      let top = stack.pop();
      let low = top[0];
      let high = top[1];
      ...
      if (low < high) {
        let pivot_index = await partition(new_arr, low, high);
        setArr([...new_arr]);
        setComparingGroup([pivot_index]);
        await sleep(animation_speed);
        ...
        if (low == high - 1){
          corrected_indexes.push(low);
          corrected_indexes.push(high);
        }
        else{
          corrected_indexes.push(pivot_index);
        }
        setSortedGroup([...corrected_indexes]);
      }
      else if(low == high){
        corrected_indexes.push(low);
        setSortedGroup([...corrected_indexes])
    ...
    console.log('sorted arr = ',new_arr);
    setComparingGroup([]);
    setSwappingGroup([]);
    // const sorted_group_arr = [];
    // for(let i = 0; i < new_arr.length; i++){
    //   sorted_group_arr.push(i);
    //   setSortedGroup([...sorted_group_arr]);
    //   await sleep(100);
    // }
    setDisableButton(false);
  ...
  const [less_than_pivot_group, setLessThanPivotGroup] = useState([]);
  ...
  const getBars = () =>{return arr.map((hheight, idx) => {
    ...
    else if(left_right_sorted_group.length !== 0 && left_right_sorted_group[0].includes(idx)){
      state = "left-sorted-group";
    }

    else if(left_right_sorted_group.length !== 0 && left_right_sorted_group[1].includes(idx)){
      state = "right-sorted-group";
    }

    else if(less_than_pivot_group.includes(idx)){
      state = "less-than-pivot";
    }
